...
bool ObjectList::count_and_delete_object_by_label(int label)
{
    DBG_PRINT("try counting and deleting object %d\n", label);
    p = head;
    while (p->next)
    {
        if (p->next->ob->get_index() == label)
...
bool ObjectList::blob_registration(Blob *blob)
{
    ...
    while (p)
    {
        ...
        // if ((distance < 120) && (size_score < 1400) && (size_score > -1400))
        if(distance < 90)
        {
...
{
    static const int exit_boundary = (IM_H) / 2 + 10;
    static const int entry_boundary = (IM_H) / 2 - 10;
    ...
    while (p)
    {
        if((p->ob->get_first_y() < entry_boundary && p->ob->get_now_y() > exit_boundary) || (p->ob->get_first_y() > exit_boundary && p->ob->get_now_y() < entry_boundary)){
            int label = p->ob->get_index();
            count_and_delete_object_by_label(label);
            p = p->next;
            continue;
        }
        if (p->ob->age_update()){
            // Age policy: if object is has been present for at least 8 frames, delete and count
            int label = p->ob->get_index();
            count_and_delete_object_by_label(label);
            p = p->next;
            continue;
    ...
    /* check if small blobs could be a fraction from a large blob in previous frame,
        if so, register it to a existing object*/
    for (int i = 0; i < n_blobs; i++)
    {
        if (matched_flag[i])
        {
    ...
    /* spawn new object from unmatched blobs of minimum size 250
    two objects cannot be spawned near one another in the same frame*/
    bool bottom = false;
    bool top = false;
    const int boundary = (IM_H) / 2 - 10;
    for (int i = 0; i < n_blobs; i++)
    {
        if (matched_flag[i] || blob_list[i].size < 250)
        {
            continue;
        }
        Blob blob = blob_list[i];
