...
HumanObject::HumanObject(int blob_label, int pos_x, int pos_y, int size)
{
    serial_num_ += 1;
    label_ = serial_num_;
    DBG_PRINT("create object %d from Blob %d at %d %d with size %d\n", label_, blob_label, pos_x, pos_y, size);
    now_.pos_x = pos_x;
    now_.pos_y = pos_y;
    now_.vel_x = 0;
    now_.vel_y = 0;
...
int HumanObject::counting()
{
    ...
    // static const int entry_boundary = (IM_H) / 2 - 20;
    static const int exit_boundary = (IM_H) / 2;
    static const int entry_boundary = (IM_H) / 2;
    DBG_PRINT("finish count of ob %d, %d->%d\n", label_, first_y_, now_.pos_y);
    if ((first_y_ < entry_boundary) && (now_.pos_y > entry_boundary))
    {
        return 1;
    }
    else if ((first_y_ > exit_boundary) && (now_.pos_y < exit_boundary))
    {
        return -1;
    }
    return 0;
...
int HumanObject::get_first_y(){
    return this->first_y_;
}

int HumanObject::get_now_y(){
    return this->now_.pos_y;
}
