...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        //debug
        event->topic = "ov2640/simul";
        event->topic_len = 13;
        //printf("topic: , data len: %d, total len: %d, offset: %d\n", event->data_len, event->total_data_len, event->current_data_offset);
        struct mqtt_msg msg;
        msg.msg = (uint8_t *) malloc(1025);
        sprintf(msg.topic, "%.*s", event->topic_len, event->topic);
        sprintf((char*) msg.msg, "%.*s", event->data_len, event->data);
        msg.offset = event->current_data_offset;
...
void mqtt_send(esp_mqtt_client_handle_t client, const char *topic, const char *data, int qos, int data_length)
{
    int msg_id;
    msg_id = esp_mqtt_client_publish(client, topic, data, data_length, qos, 0);
...
void mqtt_listen(esp_mqtt_client_handle_t client, const char *topic, QueueHandle_t *msg_q)
{
    ...
    *msg_q = xQueueCreate(75, sizeof(struct mqtt_msg));
    if (*msg_q == NULL)
    {
